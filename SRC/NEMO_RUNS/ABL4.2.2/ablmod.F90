MODULE ablmod
   ...
   USE ice, ONLY      :utauVice,vtauUice
   ! 
...
#if defined key_si3
       !REAL(wp), ALLOCATABLE, SAVE, DIMENSION(:,:  )::   utauVice,vtauUice ! SLX adaptation to BBM rheology:ice-surface tauxstress(V-point)
#endif
   !! * Substitutions
...
!===================================================================================================
   SUBROUTINE abl_stp( kt, psst, pssu, pssv, pssq,            &     ! in
              ...
              &          , ptaui_ice, ptauj_ice               &
                        , utauVice, vtauUice                  &   ! optional outputs
#endif
              &      )
!---------------------------------------------------------------------------------------------------
      ...
      REAL(wp) , INTENT(  out), DIMENSION(:,:  ), OPTIONAL ::   utauVice ! SLX adaptation to BBM rheology:ice-surface tauxstress(V-point)
      REAL(wp) , INTENT(  out), DIMENSION(:,:  ), OPTIONAL ::   vtauUice ! SLX adaptation to BBM rheology:ice-surface tauy stress(U-point)
      REAL(wp), DIMENSION(1:jpi,1:jpj       )    ::   zutau_i, zvtau_j   !!! SLX   tau_ice at F-point
#endif
      !
      ...
      ! SLX: adaptation for BBM rheology   
      ...
      IF(iom_use('taum_oce')) CALL iom_put( "taum_oce", ptaum )
      
      ! !!! SLX
     IF(iom_use('wndm_abl')) CALL iom_put( "wndm_abl", pwndm )
     !!IF(iom_use('utau_oce')) CALL iom_put( "utau_oce", ptaui )
     !!IF(iom_use('vtau_oce')) CALL iom_put( "vtau_oce", ptauj )
      ! !!!!
      ...
      ! ------------------------------------------------------------ !
      DO_2D( 0, 0, 0, 0 )
         !! SLX note: zztmp1 is uabl@ptU and zztmp2 is vabl@ptV
      ...
      END_2D
       ...
       !!! SLX
       ! at T-point for checking (to compare to taum_ice from BLK)
         DO_2D( 0, 1, 0, 1 )    ! at T point
            zutau_i(ji,jj) = rhoa(ji,jj) * pCd_du_ice(ji,jj) * (u_abl(ji,jj  ,2,nt_a) - pssu_ice(ji,jj) * rn_vfac )
            zvtau_j(ji,jj) = rhoa(ji,jj) * pCd_du_ice(ji,jj) * (v_abl(ji,jj  ,2,nt_a) - pssv_ice(ji,jj) * rn_vfac )
         END_2D
         IF(iom_use('taum_ice')) CALL iom_put('taum_ice', SQRT( zutau_i*zutau_i + zvtau_j*zvtau_j) )
         !!IF(iom_use('utau_ice')) CALL iom_put( "utau_ice", ptaui_ice )
         !!IF(iom_use('vtau_ice')) CALL iom_put( "vtau_ice", ptauj_ice )
         !!!
      CALL lbc_lnk( 'ablmod', ptaui_ice, 'U', -1.0_wp, ptauj_ice,'V', -1.0_wp )
      !

     !!! SLX : this is the equivalent of what LB added in sbcblk.F90/ice_blk_1 to work with the BBM rheology.
      !#bbm:
      IF( lbbm ) THEN
            !
            !! We also need taux@ptV and tauy@ptU (for BBM rheology)
      DO_2D( 0, 0, 0, 0 )
            !!! First we compute zztmp1 which is now uabl@ptV and zztmp2 which is vabl@ptU
         zztmp2 = 0.5_wp * ( v_abl(ji+1,jj  ,2,nt_a) + v_abl(ji,jj,2,nt_a) )
         zztmp1 = 0.5_wp * ( u_abl(ji  ,jj+1,2,nt_a) + u_abl(ji,jj,2,nt_a) )

            !! Then also compute taux@ptV and tauy@ptU ( BBM)
            !! I used the same variable names as LB here even though it is a bit confusing with GS's initial notations in this
            !routine but to sum up: putaui_v is tauy@ptU and pvtaui_u is taux@ptV
         vtauUice(ji,jj) = 0.5_wp * (  rhoa(ji+1,jj) * pCd_du_ice(ji+1,jj)             &
            &                      +    rhoa(ji  ,jj) * pCd_du_ice(ji  ,jj)  )          &
            &         * ( zztmp2 - pssv_ice(ji,jj) * rn_vfac )
         utauVice(ji,jj) = 0.5_wp * (  rhoa(ji,jj+1) * pCd_du_ice(ji,jj+1)             &
            &                      +    rhoa(ji,jj  ) * pCd_du_ice(ji,jj  )  )          &
            &         * ( zztmp1 - pssu_ice(ji,jj) * rn_vfac )            
      END_2D
      CALL lbc_lnk( 'ablmod', vtauUice, 'V', -1._wp, utauVice, 'U', -1._wp )
      END IF
      !#bbm.
      !
